# Aim of this script is to obtain GWAS summary statistics for max H2 component 
...
maxH=GWAS_linear_combination_Z_based(a=as.numeric(aa[1,]),Z = z,covm=as.matrix(covm),N=sample_size, eaf=gwas_reordered[[1]]$eaf)

maxH=mutate(maxH,Z=b/se,p=pchisq(Z^2,1,low=F))
maxH=mutate(maxH,SNP=gwas_reordered[[1]]$rs_id)
maxH=mutate(maxH,A1=gwas_reordered[[1]]$ea,A2=gwas_reordered[[1]]$ra,chr=gwas_reordered[[1]]$chr,pos=gwas_reordered[[1]]$bp,
...
data.table::fwrite(
	maxH, 
	row.names=F,
	file = '../../data/01_anthropometry_results/five_traits/linear_combination/maxH_GWAS.txt',
	sep = '\t')
