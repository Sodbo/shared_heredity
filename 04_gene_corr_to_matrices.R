...
path_to_result_directory<-'../data/anthropometry_results/four_traits/'
...
write.table(corr_matrix, paste0(path_to_result_directory,'gene_corr_matrix_new.txt'), row.names=F, quote=F)
cov_matrix<-dcast(united_corr_table, gwas_id_1 ~ gwas_id_2, value.var = "cov")
colnames(cov_matrix)<-c('',colnames(cov_matrix)[c(-1)])
write.table(cov_matrix, paste0(path_to_result_directory,'gene_cov_matrix_new.txt'), row.names=F, quote=F)
