...
library(data.table)
library(dplyr)
source("../00_core_functions/linear_combination_v3.R", chdir=T)
...
path_to_result_directory<-'../../data/02_Lipids/GWAS/scaled_filtered'
gwas_files<-list.files(path_to_result_directory, full.names=T, pattern="ID_\\d+.csv")
gwas<-lapply(gwas_files, fread)

aa<-read.table('../../data/02_Lipids/alphas.txt', row.names=1)
phem<-read.table('../../data/02_Lipids/pheno_corr_matrix.txt', row.names=1,  check.names=F)
gcov<-read.table('../../data/02_Lipids/gene_cov_matrix.txt', row.names=1,  check.names=F)
...
slope=apply(n_traits,function(x) cov_gi_alpha(a=alphas,i=x,covm=as.matrix(gcov))/H2(alphas,covm=gcov,phem=phem))
...
lapply(n_traits, function(x) data.table::fwrite(
		tr_sh_gwas[[x]], 
		row.names=F,
		file = paste0('../../data/02_Lipids/linear_combination/Tr',colnames(gcov)[x],'-SH_GWAS.txt'),
		sep = '\t')
	)
